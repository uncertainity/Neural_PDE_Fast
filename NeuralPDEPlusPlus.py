...
class NeuralPDE(nn.Module):
    ...
    def __init__(self,input_size,hidden_size,output_size,model = "Wave"):
        ...
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.gru_1 = GRU(self.input_size,self.hidden_size)
        self.gru_2 = GRU(self.input_size,self.hidden_size)
        
        if model == "Wave":
            self.pdefunc = Wave(self.gru_1,self.gru_2,self.hidden_size)
        elif model == "Heat":    
            self.pdefunc = Heat(self.gru_1,self.hidden_size)
